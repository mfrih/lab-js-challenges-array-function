...
function howManyTimes(arrayOfWords, referenceWord) {
  let count = 0;
...
function filterOut(originalArray, arrayToRemove) {
  ...
  for (const word of originalArray) {
    if (!arrayToRemove.includes(word)) {
      arr.push(word);
    }
  }
  return arr;
...
function uniquifyArray(Arr) {
  ...
  for (const word of Arr) {
    if (nonDupArray.indexOf(word) === -1) {
      nonDupArray.push(word);
    }
  }
  return nonDupArray;
}
